...
- Sistema privado: Fase 2.6 implementada, Fase 3.0 validada en DEV, Fase 3.1 documentada, Fase 3.2 implementada, Fase 3.2.1 validada técnicamente y bugs de repintado en Admin > Clientes, Admin > Órdenes > Nueva orden y Admin > Dashboard corregidos con signals; `/app` y `/app/dashboard` siguen privados, `/dashboard` no es ruta privada real, `/app/ordenes` se extendió sin panel duplicado con impresión de etiqueta interna y etiqueta de entrega.
...
- Correcciones zoneless 2026-07-02 y 2026-07-03 en Clientes, Edición de cliente, Nueva orden, Dashboard Admin y Detalle de orden no modificaron backend, contratos API, base de datos, migraciones, `AuthService`, guards, cookies, XSRF, deploy ni dependencias; no se agregó `zone.js` ni se quitó `withFetch()`.
...
Commit/push a rama `dev`, desplegar a VPS DEV y validar Fase 3.2.1 con impresora térmica real antes de iniciar Fase 3.3 - entrega/repartidor mobile-first.

Alcance sugerido inmediato: probar etiqueta interna 76 x 51 mm y etiqueta entrega 102 x 51 mm en DEV con impresora real, ajustar configuración de navegador/driver/rollo si hace falta y documentar evidencia manual. La siguiente fase funcional mayor debe diseñar entrega/repartidor mobile-first con permisos, modelo, endpoints y UI bajo `/app`.
