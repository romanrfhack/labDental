...
Commit/push a rama `dev`, desplegar a VPS DEV y validar Fase 3.3/3.3.1 de usuarios/roles con Admin y usuario sin permiso antes de iniciar entrega/repartidor mobile-first.

Alcance sugerido inmediato: validar en DEV `/app/admin/usuarios`, creación de usuario, asignación de `Repartidor`, activación/desactivación y `/app/admin/roles` readonly; en paralelo, probar etiqueta interna 76 x 51 mm y etiqueta entrega 102 x 51 mm con impresora real cuando el driver quede disponible. La siguiente fase funcional mayor debe diseñar entrega/repartidor mobile-first con permisos, modelo, endpoints y UI bajo `/app`.
...
Backlog futuro separado: Fase 3.4 entrega/repartidor mobile-first y Fase 3.5 administración de catálogo, precios e imágenes.
