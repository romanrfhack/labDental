...
- Fase 3.5.4.0: **CERRADA** el 2026-08-08. Define `${LDT_APP_ROOT}/shared/catalog-images` fuera de releases, ruta pública `GET /api/catalog/images/{fileName}`, `ImagePath` `/api/catalog/images/{fileName}`, POST/DELETE por producto, máximo 2 MB, formatos permitidos, validación de extensión/MIME/firma, nombre único, bloqueo de path traversal, permisos de filesystem y estrategia de backup/restore.
- Fase 3.5.4.1: **CERRADA EN DEV** el 2026-08-08. Backend upload/reemplazo/desasociación desplegado en commit `1b0384c414b54f541394dbe0e2f1e4a4d9329e93`; storage persistente preparado para `www-data`, POST multipart/XSRF, GET público y DELETE validados end-to-end, sin migración ni dependencias nuevas.
- Fase 3.5.4.2: **CERRADA EN DEV** el 2026-08-08. UI desplegada en commit `f9acb0dfa973bd131ab2850c69105c4a90d84470`, GitHub Actions `success`; creación/edición de producto, selección, preview, upload, reemplazo, render público y quitar imagen quedaron aprobados.
- Fase 3.5.4 completa: **QA END-TO-END APROBADO EN DEV** el 2026-08-08. El archivo sobrevivió al release backend posterior `dev-44-8c2f92b` y continuó sirviéndose/mostrándose. Después de DELETE, `/api/catalog/public` y `/catalogo` dejaron de referenciar/mostrar la imagen, mientras el archivo físico y su GET directo permanecieron disponibles; esto es intencional porque DELETE desasocia y no elimina físicamente.
- Backlog futuro de imágenes, no bugs de 3.5.4: inventario de archivos huérfanos, política de retención, limpieza segura de no referenciados, backup automatizado de `shared/catalog-images`, posible conversión/recompresión WebP, upload de imagen de sección y galería múltiple/CDN/cloud storage.
...
Validar Fase 3.2 en DEV con impresora térmica real, incluyendo escala y márgenes para etiquetas internas de 76 x 51 mm y de entrega de 102 x 51 mm. Es el siguiente pendiente técnico canónico ya implementado que requiere evidencia física.

En paralelo, cerrar la validación manual del usuario QA limitado y `/app/access-denied` si aún falta evidencia con una cuenta limitada real sin `reports.view`. La prueba forzada del fallback de `/catalogo` con API bloqueada/offline continúa como cobertura manual opcional.

Fase 3.5.4 queda cerrada por QA end-to-end en DEV; sus mejoras de ciclo de vida de archivos y almacenamiento avanzado permanecen como backlog futuro, no como defectos.
