...
- Fase 3.4.3.1: redirect post-login por permisos y reintento de entrega fallida. Implementada el 2026-07-05; sin `returnUrl` interno válido el login usa la ruta inicial por permisos (`reports.view` primero y `deliveries.view -> /app/entregas` para `Repartidor`), `/app/access-denied` enlaza a `Ir a mi inicio`, y `PATCH /api/deliveries/{id}/retry` permite pasar `FailedDelivery` a `OutForDelivery` sin cambiar `WorkOrder.Status`. Admin/operación reintenta con `deliveries.update`; repartidor asignado reintenta con `deliveries.complete`. No crea migraciones, no agrega dependencias y no toca cookies/XSRF.
...
Iniciar QA DEV de Fase 3.4.3.1 y después continuar con pulido mobile-first del flujo de reparto, tomando como fuentes `docs/08-qa/delivery-admin-ui-qa.md`, `docs/08-qa/driver-mobile-qa.md` y `docs/08-qa/delivery-api-qa.md`.

Alcance sugerido inmediato: validar que un `Repartidor` sin `returnUrl` entra a `/app/entregas`, que `/app/access-denied` apunta a su inicio por permisos, que Admin reintenta una entrega `FailedDelivery` desde `/app/ordenes/:id`, que el repartidor asignado reintenta desde `/app/entregas/:id`, que el retry deja la entrega en `OutForDelivery` sin cambiar `WorkOrder.Status`, que luego puede cerrarse como `Delivered` con `recipientName` o volver a `FailedDelivery` con último `failedReason`, y revisar responsive en celular real o DevTools móvil.
